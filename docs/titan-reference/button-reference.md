...
\<Align\> | (D9 only) Allows copying of attributes between fixtures. On other consoles this function is accessed using \<ML Menu\>. See [Align Fixtures](../controlling-fixtures/using-the-select-buttons-and-wheels.md/#align-fixtures).
\<All\> | Used to cancel odd/even selection on a range of fixtures. See [Selecting fixtures using a pattern](../controlling-fixtures.md/#selecting-fixtures-using-a-pattern).
\<And\> | Used when selecting ranges of items using the numeric keypad (e.g. Fixture 1 Through 5 And 7). See [Selecting fixtures and dimmers by number](../controlling-fixtures.md/#selecting-fixtures-and-dimmers-by-number-channel).
\<Assign\> | (D9 and ST only) Allows you to reassign attributes controlled by the trackball and (on D9 only) intensity wheel.  
\<Avo\> | Acts as a "Shift" button when held down to give access to different functions. 
\<Back\> |
\<Beam\> | Selects the Beam attribute group so the wheels will control Zoom, focus etc. Also used for creating masks. See [Changing Attributes using the Wheels](../controlling-fixtures/changing-fixture-attributes.md/#changing-attributes-using-the-wheels). 
...
\<Enter/B\> | (D9 only) Enters Scene Master preset mode.
\<Exit\> | Leaves the current menu and goes back up the menu structure.
\<Exit/A\> | (D9 only) Exits Scene Master preset mode and goes back to Live.
\<FX\> | (D9 only) Selects the Shape/FX menu to run shapes, key frame shapes or pixel map effects.
\<FX (wheels)\> | (D9 only) Sets the wheels into FX mode to control shape size, speed, spread.
\<Fade/Delay\> | (D9 only) Sets the wheels into Fade/Delay mode to control fade and delay times.
\<Fan\> | Starts Fan mode so the wheels spread out attributes rather than setting values.
\<Fix +1\> | Selects the next fixture out of a range.
\<Fix -1\> | Selects the previous fixture out of a range.
\<Fixture\> | Opens the fixture menu.
\<Go\> | Runs the next cue of the connected playback.
\<Go page\> | Type a number to jump to Page ``<n>`` on the associated fader bank (consoles with multiple fader banks may have several of these buttons).
\<Gobo\> | Selects the Gobo attribute group so the wheels control gobo select, rotation etc
\<Group\> | Opens the group menu.
\<Hi light\> | Starts Hi-light mode which visually shows the selected fixture on stage by increasing its intensity or setting colour.
\<Include\> | (followed by playback Select) Loads the contents of a playback back into the programmer for further editing or re-use.
\<Intensity\> | Selects the Intensity attribute group so the wheels control Dimmer, Shutter etc
\<Keyboard\> | (D9 only) Shows the touch screen keyboard.
\<Latch\> | (D9 only) Latches the current menu so that it remains active when you complete the action. Useful for copy, move etc.
\<Latch Menu\> | Latches the current menu so that it remains active when you complete the action. Useful for copy, move etc.
\<Left\> | (D9 / ST only) Left mouse button when trackball is in mouse mode.
...
\<Palette\> | Opens the Palette menu.
\<Patch\> | Opens the Patch menu.
\<Playback\> | (D9)  (Arena) Opens the Playback View window
\<Playback (wheels)\> | (D9 only) Sets the wheels to playback control mode.
\<Position\> | Selects the Position attribute group so the wheels control pan, tilt etc
\<Preload/Auto\> | (D9 only) Preload the Scene Master preset.
\<Prev Step\> | Moves the next cue in the connected cue list back by one.
\<Record\> | Enter Record mode.
\<Record Step\> | Record the programmer into the current active cue in the connected cue list.
\<Release\> | Used to override the LTP behaviour of attributes when a playback is killed.
\<Reset\> | (D9 only) Clear the preset state of the Scene Master.
\<Review\> |
\<Right\> | (D9 / ST only) Right mouse  button when trackball in mouse mode.
\<Select If\> | Selects all fixtures which have intensity above zero.
\<Set\> | This button has become the \<Time\> button.
\<Shape\> | Selects the Shape/FX menu to run shapes, key frame shapes or pixel map effects.
\<Size/Position\> | Moves the current window around the different size and position options. If \<Avo\> held also, moves the window to the other screen.
