...
In order to save a **Prism Player** project it is possible to click on `File > Save` or use the `Ctrl+S` shortcut.  
...
In order to load a **Prism Player** project it is possible to click on `File > Open` or use the `Ctrl+O` shortcut.  
