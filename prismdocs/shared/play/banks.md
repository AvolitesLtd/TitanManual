...
<!-- Player & Zero -->
<p style={{display: (`{{PRISM-APP-LOWER}}` === 'player' || `{{PRISM-APP-LOWER}}` === 'zero') ? 'inline-block' : 'none'}}>
...
<p style={{display: (`{{PRISM-APP-LOWER}}` === 'prism' || `{{PRISM-APP-LOWER}}` === 'zero') ? 'inline-block' : 'none'}}>
